...
async def generate_tts_for_session(session_dir: Path) -> list[dict]:
    ...
    for scene in scenes:
        for seg in scene.get("subtitles", []):
            ...
            logger.info("Generating TTS: %s → %s", text[:30], filename)
    ...
    logger.info("TTS complete. Total frames: %d", current_frame)
    return scenes
...
async def _synthesize(text: str, output_path: Path) -> None:
    ...
    model = "cosyvoice-v1"
    voice = settings.tts_voice  # e.g. "longshuo"
...
def _get_audio_duration_ms(path: Path) -> int:
    ...
    try:
        ...
        # Approximate: 16kbps mp3 ≈ 2000 bytes/sec

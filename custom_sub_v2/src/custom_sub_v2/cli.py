"""CLI interface for custom_sub_v2."""
...
def submit(article_file, requirements, width, height, fps, wait):
    ...
    async def _submit():
        ...
        from custom_sub_v2.services.pipeline import start_pipeline
            ...
            while True:
                ...
                if status in ("completed", "failed", "stopped"):
                    ...
                    if status == "failed":
                        ...
                        sdir = session_manager.session_dir(project_id)
...
def progress(project_id):
    ...
    async def _progress():
        ...
        p = await session_manager.get_progress(project_id)
        click.echo(f"Project:  {p.project_id}")
        ...
        click.echo("Steps:")
        for step in p.steps:
            icon = {"pending": ".", "in_progress": ">", "completed": "+", "failed": "x"}.get(step.status, "?")
...
def resume(project_id, prompt):
    """Resume a session with a modification prompt."""
    async def _resume():
        from custom_sub_v2.services.pipeline import resume_pipeline

        await resume_pipeline(project_id, prompt)
...
def list_sessions():
    ...
    async def _list():
        ...
        for item in items:
            click.echo(
                f"  {item.project_id}  [{item.status.value:20s}]  "
...
def stop(project_id):
    ...
    async def _stop():
        from custom_sub_v2.services.pipeline import stop_pipeline
...
def delete(project_id):
    """Delete a session."""
    async def _delete():
        from custom_sub_v2.services.pipeline import stop_pipeline, is_running
        from custom_sub_v2.services.session import session_manager
...
def serve(host, port):
    ...
    import uvicorn
    from custom_sub_v2.main import app
